...
const Container = styled(Flex)`
  ...
  background: linear-gradient(0deg, rgba(39, 38, 44, 0.4), rgba(39, 38, 44, 0.4)),
    linear-gradient(180deg, #20283a 0%, #3d546c 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #20283a 0%, #3d546c 100%);
  }
`
...
const PhishingWarningBanner: React.FC<React.PropsWithChildren> = () => {
  ...
  const warningTextAsParts = useMemo(() => {
    const warningText = "please make sure you're visiting https://app.knightswap.financial - check the URL carefully."
    return warningText.split(/(https:\/\/knightswap.financial)/g)
  }, [t])
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://knightswap.financial'}
          color={text === 'https://knightswap.financial' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
