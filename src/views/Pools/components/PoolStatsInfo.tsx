...
import { bsc, fantomOpera } from '@pancakeswap/wagmi/chains'
...
const PoolStatsInfo: React.FC<React.PropsWithChildren<ExpandedFooterProps>> = ({
  ...
  return (
    <>
      ...
      {vaultKey && (
        <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <LinkExternal href="https://knights.gitbook.io/darkknight/" bold={false} small>
            {t('View Tutorial')}
          </LinkExternal>
        </Flex>
      )}
      {poolContractAddress && (
        <Flex mb="2px" justifyContent={alignLinksToRight ? 'flex-end' : 'flex-start'}>
          <LinkExternal
            href={`${fantomOpera.blockExplorers.default.url}/address/${
